...
namespace MD3_Droids
{
    public class DroidChassisPartDef : BodyPartDef
    {
        ...
        public int addedPartHealth = -1;

        public string GetTooltip()
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine(description);
            if (addedPartHealth > 0)
            {
                sb.AppendLine();
                sb.AppendLine($"Added health: {addedPartHealth}");
            }
            ...
            if (capMods.Count > 0)
            {
                ...
                {
                    sb.AppendLine($"   {cap.capacity.LabelCap} {(cap.offset > 0 ? "+" : "-")}{cap.offset * 100f}%");
                }
            }
